...
func ClientFinished() {
	...
	skemsg := HashshakeMessage()
	sm3_hash := HashSM3(skemsg)
	fmt.Println("Hash->", hex.EncodeToString(sm3_hash))

	prf_out := PRF(masterkey, []byte(finished_label), sm3_hash)
...
func HashshakeMessage() []byte {
	...
	hashshakeMsg := ClientHello + ServerHello + Certificate + ServerKeyExchange + ServerHelloDone + ClientKeyExchange
